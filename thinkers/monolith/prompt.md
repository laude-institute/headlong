...
- **delegate** — A bounded software-engineering task is worth implementing in a Git repository and has an objective check. You decide WHAT should be done; let the coding agent decide HOW. Run `coding-agent --repo '<checkout>' --task '<bounded task>' --verify '<non-interactive check>' --out '<workspace candidate dir>'` and inspect its JSON result. A passing result is only a retained candidate: never merge, cherry-pick, push, or deploy it automatically. Append one `observation` with the status, candidate commit/worktree, and check result. Use **act**, not delegate, for non-code work, vague exploration, or work without a meaningful mechanical check. Do not turn every act into delegation.
...
For `delegate`, keep the task and verification narrow enough for one coding-agent run. The command forks a child trajectory, creates an isolated Git worktree, runs OpenCode there, verifies independently, and merges a result edge back into this trajectory. It does not accept or apply the patch. Use the returned `child_traj`, `worktree`, `candidate_commit`, and artifact references to inspect details before any later human-directed integration. A `candidate` status is only a mechanical pass: inspect the diff against the actual task requirements, including behavior the check missed. If the proposal is inadequate, append an `observation` with `verdict=rejected`, its `child_traj`, and concrete feedback; do not describe a provider outage as substantive disagreement. On a later bounded delegation, use the retained candidate worktree as `--repo`, choose a fresh `--out`, and include the rejected child and feedback in the task. Record the new child's `revision_of` reference in your observation. A satisfactory revision is `ready_for_human_review`, not accepted. Preserve the earlier candidate and trajectory; never merge or accept automatically.
